---
title: 'R documentation: transitioning from odin interface to R'
...
- *[Introduction to R for IDM](https://mrc-ide.github.io/R-for-IDM-Intro/)*: This documentation is part of Imperial College's online materials, spcifcally directed to infectious diseaase modelling.
...
## A comparative example of `odin` interface and `odin` in R

We will use a simple SIR closed model as example for of how results obtained using the short-course interface can be reproduced in R and Rstudio.
...
Another feature of the interface is the "Sensitivity" panel. Here, you can explore the sensitivity of model outputs to variations in parameter values. This can be presented in different graphical options. Here, a plot showing the traces of "Number Infected" over time from 100 simulations exploring a +/- 20% variation in the value of *beta*.
...
Since we have a set of results, we can plot them to see the model trajectories over time. We will start by plotting using R base graphics and then we will see the same using `ggplot2.`
...
```{r echo=TRUE, message=FALSE, warning=FALSE}
# Melt data frame to be compatible with ggplot
ymelt <- melt(y, id.vars = "t")

# Plot the model states over time with ggplot
...
(note that for this case, the values are `t` and `sir` here are found in the "Global environment" - i.e., the values that you have already saved in your R session. For more information on this programming style see [the functional programming chapter in Advanced R](http://adv-r.had.co.nz/Functional-programming.html))
...
```{r}
yy <- as.data.frame(output)
yy[["t"]] <- t
yymelt <- melt(yy, id.vars = "t")
...
ggplot(data = yymelt) +
  ...
  scale_color_manual(values = rep(cols[2], sims)) +
  geom_line()
```
